...
export function createMarkdownSerializer() {
    ...
    return new MarkdownSerializer({
        ...
        code: {
            open(_state: MarkdownSerializerState, _mark: Mark, parent: Node, index: number) { return backticksFor(parent.child(index), -1); },
            close(_state: MarkdownSerializerState, _mark: Mark, parent: Node, index: number) { return backticksFor(parent.child(index - 1), 1); },
            escape: false
        },
    });
}