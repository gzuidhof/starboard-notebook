...
export function parseNotebookContent(notebookContentString: string) {
    ...
    for (const line of allLines.slice(cellLinesStartIndex)) {
      ...
      if (cellDelimiterMatches !== null) { // Start a new cell - here we parse the initial line that starts a new cell
        ...
        if (!cellTypeMatches) { // No cell type is defined like [javascript]
          if (line.startsWith("%%")) {// Deprecated: here for old format compatibility - no square brackets are used here.
        ...
        if (!currentCell) { // This should never happen as it would otherwise have been frontmatter
